...
def do_risk_acceptance_replication(new_finding, *args, **kwargs):
    """Replicate risk accepted findings across product when using dedup within engagement

    Mark finding as risk accepted if the same finding was previously accepted
    in other engagement from the same product.
    ...
    if new_finding.test.engagement.deduplication_on_engagement:
        ...
        #      ) and test_type
        #      and hash_code
        #    as the new one and also are risk accepted
        #    from other engagements
        #    (this is "cond1")
        # ---------------------------------------------------------

        product_findings_cwe = (
        ...
        total_findings = (
            Finding.objects.filter(
                Q(id__in=product_findings_cwe) | Q(id__in=product_findings_title)
            ).prefetch_related(
                'endpoints',
                'test',
                'test__engagement',
        ...
        deduplicationLogger.debug(
            "Found " +
            str(len(product_findings_cwe)) + " findings with same cwe and hash, " +
            str(len(product_findings_title)) + " findings with same title and hash: " +
        ...
        for find in total_findings.order_by('id'):
            ...
            if find.endpoints.count() != 0 and new_finding.endpoints.count() != 0:
                ...
                if str(find.line) == str(new_finding.line) and find.file_path == new_finding.file_path:
                    ...
                    deduplicationLogger.debug("no endpoints on one of the findings and file_path doesn't match; Risk acceptance replication will not occur")

            else:
                deduplicationLogger.debug("no endpoints on one of the findings and the new finding is either dynamic or doesn't have a file_path; Risk acceptance replication will not occur")

            deduplicationLogger.debug(
                'risk acceptance replication flags for new finding (' + ('dynamic' if new_finding.dynamic_finding else 'static') + ') ' + str(new_finding.id) +
                ' and existing finding ' + str(find.id) +
                ' flag_endpoints: ' + str(flag_endpoints) + ' flag_line_path:' + str(flag_line_path)
            )

            # ---------------------------------------------------------
            # 3) Findings are marked as risk accepted if 
            #    (cond1 is true)
            ...
            if flag_endpoints or flag_line_path:
                try:
                    ...
                    new_finding.risk_accepted = True
